...
function calcVolume(week, day) {
  let vol = 0;
  const d = PROGRAM[day]; if (!d || d.mobilite) return 0;
...
function calcScore(week, day) {
  const d = PROGRAM[day]; if (!d || d.mobilite) return d && S.cache[`mobilite_${week}_${day}`] ? 80 : 0;
  const allExos = [...(d.exos || []), ...(d.abdosExos || [])];
...
function calcStreak() {
  ...
  for (let w = S.week; w >= 1; w--) {
    ...
    for (const dk of Object.keys(PROGRAM)) {
      const d = PROGRAM[dk];
      if (d.mobilite) { if (S.cache[`mobilite_${w}_${dk}`]) { has = true; break; } }
      else if (d.exos && d.exos.some(exo => gv(w, dk, exo.id, 0, 'kg'))) { has = true; break; }
    }
...
function checkAchievements() {
  ...
  const mobiliteCount = parseInt(S.cache['mobilite_sessions'] || '0');
  const checks = {
    ...
    gainage_10: gainageCount >= 10, mobilite_10: mobiliteCount >= 10,
  };
  const newly = [];
...
function updateTopbar() {
  ...
  const vol = day.mobilite ? 0 : calcVolume(S.week, S.day);
  document.getElementById('tb-vol').textContent = day.mobilite ? '~22 min' : `${Math.round(vol).toLocaleString()} kg`;
  const cardio = S.cardioLog[`${S.week}_${S.day}`];
  const kcal = day.mobilite ? 120 : calcEstimatedCalories(day.type, vol, S.sessionDuration, cardio);
...
function renderWeekStrip() {
  ...
  strip.innerHTML = Object.entries(PROGRAM).map(([dk, day]) => {
    const isMobilite = !!day.mobilite;
    const hasPR = !isMobilite && day.exos && day.exos.some(exo => exo.series.some((_, si) => isPR(exo.id, S.week, dk, si)));
    const hasDone = isMobilite ? !!S.cache[`mobilite_${S.week}_${dk}`] : (day.exos && day.exos.some(exo => gv(S.week, dk, exo.id, 0, 'kg')));
...
function renderSeance() {
  ...
  // Jour mobilité jeudi — écran spécial
  if (day.mobilite) {
    const done = !!S.cache[`mobilite_${S.week}_${S.day}`];
    document.getElementById('tab-seance').innerHTML = `<div class="fade-in">
      <div class="insight-card" style="background:var(--bg1);border:0.5px solid var(--bd);margin-bottom:12px">
        <span class="ic-icon">🧘</span>
        <div><div class="ic-title">${day.name}</div><div class="ic-sub">${day.sub} · ~22 min</div></div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:16px">
        ${['🦵 Hanches & Fléchisseurs','🫁 Thoracique & Épaules','🔙 Ischios & Dos','🦶 Cheville & Mollets'].map(b => `<div style="background:var(--bg2);border-radius:10px;padding:10px 12px;font-size:11px;color:var(--t2);font-weight:600">${b}</div>`).join('')}
      </div>
      <div class="gainage-row${done ? ' done' : ''}" onclick="openMobilite()">
        <div class="gr-left">
          <div class="gr-name">${done ? 'Mobilité terminée ✓' : 'Démarrer la mobilité'}</div>
          <div class="gr-sub">${done ? '17 exercices complétés · ~22 min' : '4 blocs · 17 exercices · 60–90s par position'}</div>
        </div>
        <div class="gr-right">
          <span class="gr-prog">${done ? '✓' : '0/17'}</span>
          <div class="dots"><div class="dot ${done ? 'done' : ''}"></div></div>
        </div>
      </div>
    </div>`;
    return;
  ...
  // Exercices principaux avec gestion supersets
  let lastGroup = null;
  day.exos.forEach((exo, ei) => {
    if (exo.supersetGroup && exo.supersetGroup !== lastGroup) {
      html += `<div class="section-hdr" style="background:var(--bl-d);border:0.5px solid #1A2440;margin-bottom:0"><span class="sh-icon">⚡</span><span class="sh-label" style="color:var(--bl)">Superset ${exo.supersetGroup}</span><span class="sh-badge" style="background:var(--bl-d);color:var(--bl);border:0.5px solid #1A2440">Enchaîner sans pause</span></div>`;
      lastGroup = exo.supersetGroup;
    } else if (!exo.supersetGroup) {
      lastGroup = null;
    }
  ...
  // Section abdos — sur tous les jours qui ont des abdosExos
  if (day.abdosExos && day.abdosExos.length > 0) {
...
// ── MOBILITÉ SCREEN ───────────────────────────────────────────────────────────
function openMobilite() {
  S.mobilite = { step: 0, phase: 'work', elapsed: 0, startTime: null, interval: null, completed: 0 };
  requestWakeLock();
  if (!S.sessionStart) startSessionChrono();
  document.getElementById('mobilite-screen').classList.add('on');
  renderMobilite();
  startMobiliteTimer();
}

function closeMobilite() {
  stopMobiliteTimer();
  releaseWakeLock();
  document.getElementById('mobilite-screen').classList.remove('on');
  renderSeance(); updateTopbar();
}

function stopMobiliteTimer() {
  clearInterval(S.mobilite.interval); S.mobilite.interval = null;
}

function startMobiliteTimer() {
  stopMobiliteTimer();
  S.mobilite.startTime = Date.now();
  S.mobilite.interval = setInterval(() => {
    const elapsed = Math.floor((Date.now() - S.mobilite.startTime) / 1000);
    const current = MOBILITE_PROTOCOL[S.mobilite.step];
    if (!current) return;
    const limit = S.mobilite.phase === 'work' ? current.work : current.rest;
    S.mobilite.elapsed = elapsed;
    if (elapsed >= limit) advanceMobilite();
    else updateMobiliteUI(elapsed, limit);
  }, 250);
}

function advanceMobilite() {
  stopMobiliteTimer();
  if (S.mobilite.phase === 'work') {
    playEndBeep();
    S.mobilite.completed++;
    if (S.mobilite.step >= MOBILITE_PROTOCOL.length - 1) { finishMobilite(); return; }
    S.mobilite.phase = 'rest';
    S.mobilite.startTime = Date.now(); S.mobilite.elapsed = 0;
    renderMobilite(); startMobiliteTimer();
  } else {
    S.mobilite.step++;
    S.mobilite.phase = 'work';
    S.mobilite.startTime = Date.now(); S.mobilite.elapsed = 0;
    playStartBeep(); renderMobilite(); startMobiliteTimer();
  }
}

function finishMobilite() {
  stopMobiliteTimer();
  S.cache[`mobilite_${S.week}_${S.day}`] = '1';
  S.cache['mobilite_sessions'] = String((parseInt(S.cache['mobilite_sessions'] || '0')) + 1);
  saveLocal();
  showToast('🧘 Mobilité terminée — 22 min !');
  checkAchievements();
  document.getElementById('mobilite-screen').classList.remove('on');
  renderSeance(); updateTopbar();
}

function skipMobiliteStep() {
  stopMobiliteTimer();
  S.mobilite.step++;
  if (S.mobilite.step >= MOBILITE_PROTOCOL.length) { finishMobilite(); return; }
  S.mobilite.phase = 'work';
  S.mobilite.startTime = Date.now(); S.mobilite.elapsed = 0;
  renderMobilite(); startMobiliteTimer();
}

function updateMobiliteUI(elapsed, limit) {
  const isWork = S.mobilite.phase === 'work';
  const remaining = limit - elapsed;
  const pct = elapsed / limit;
  const circumference = 329;
  const offset = circumference * (1 - pct);
  const fillEl = document.querySelector('.m-fill');
  if (fillEl) {
    fillEl.style.strokeDashoffset = offset;
    fillEl.style.stroke = isWork ? (remaining <= 10 ? 'var(--am)' : '#7F77DD') : 'var(--gr)';
  }
  const timeEl = document.querySelector('.m-time');
  if (timeEl) {
    timeEl.textContent = remaining;
    timeEl.className = 'm-time ' + (isWork ? (remaining <= 10 ? 'warn' : '') : 'rest');
  }
  if (remaining === 3 || remaining === 2 || remaining === 1) {
    if (isWork) playCountdownBeep(); else playRestCountdownBeep();
  }
}

function renderMobilite() {
  const step = S.mobilite.step;
  const phase = S.mobilite.phase;
  const current = MOBILITE_PROTOCOL[step] || MOBILITE_PROTOCOL[MOBILITE_PROTOCOL.length - 1];
  const isWork = phase === 'work';
  const totalSteps = MOBILITE_PROTOCOL.length;
  const pctDone = step / totalSteps;
  // Grouper par bloc pour mini-nav
  const blocs = [...new Set(MOBILITE_PROTOCOL.map(e => e.bloc))];
  const currentBloc = current.bloc;

  document.getElementById('m-body').innerHTML = `
    <div class="g-prog-bar"><div class="g-prog-fill" style="width:${Math.round(pctDone * 100)}%;background:#7F77DD"></div></div>
    <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">
      ${blocs.map(b => `<div style="font-size:10px;padding:3px 8px;border-radius:10px;font-weight:600;background:${b===currentBloc?'#2A2A50':'var(--bg2)'};color:${b===currentBloc?'#AAA8F0':'var(--t3)'}">${b}</div>`).join('')}
    </div>
    <div class="g-phase">
      <div class="g-phase-label" style="color:${isWork ? '#AAA8F0' : 'var(--gr)'}">${isWork ? 'Étirement · Position' : 'Repos'} · ${step + 1}/${totalSteps}</div>
      <div class="g-exo-name">${current.name}</div>
      <div class="g-exo-sub">${isWork ? `${current.work}s · Tiens la position` : `${current.rest}s · Prépare : ${MOBILITE_PROTOCOL[step + 1]?.name || 'Terminé'}`}</div>
    </div>
    <div class="g-circle-wrap">
      <svg class="g-svg" viewBox="0 0 110 110">
        <circle class="g-bg" cx="55" cy="55" r="52"/>
        <circle class="m-fill g-fill" cx="55" cy="55" r="52" style="stroke:${isWork ? '#7F77DD' : 'var(--gr)'};stroke-dashoffset:0"/>
      </svg>
      <div class="g-center">
        <div class="m-time g-time ${isWork ? '' : 'rest'}">${isWork ? current.work : current.rest}</div>
        <div class="g-time-l">${isWork ? 'sec' : 'repos'}</div>
      </div>
    </div>
    ${isWork ? `<div class="g-tip" style="border-left:2px solid #7F77DD"><div class="g-tip-l">Technique</div><div class="g-tip-t">${current.tip}</div></div>` : ''}
    <div class="g-series" style="max-height:160px;overflow-y:auto">
      ${MOBILITE_PROTOCOL.map((g, i) => {
        ...
        return `<div class="g-serie-row${isAct ? ' active' : isDone ? ' done' : isRest ? ' rest' : ''}">
          ...
          <span class="g-sname">${g.name}</span>
          <span class="g-sdur">${isDone ? '✓ ' + g.work + 's' : g.work + 's'}</span>
          <div class="g-sic" style="background:${isDone ? 'var(--gr-d)' : isAct ? '#2A2A50' : 'var(--bg2)'};color:${isDone ? 'var(--gr)' : isAct ? '#AAA8F0' : 'var(--t4)'}">
            ${isDone ? '✓' : isAct ? '▶' : '○'}
          </div>
        </div>`;
      }).join('')}
    ...
    <div style="display:flex;gap:8px;margin-top:4px">
      <button class="g-btn" style="background:#2A2A50;color:#AAA8F0;border:0.5px solid #3A3A60" onclick="advanceMobilite()">
        ${isWork ? 'Position suivante' : 'Continuer'}
      </button>
      <button style="padding:13px 12px;background:var(--bg1);border:0.5px solid var(--bd);border-radius:8px;font-size:11px;font-weight:700;color:var(--t3);cursor:pointer" onclick="skipMobiliteStep()">Passer</button>
    </div>`;
}
...
function showEndScreen() {
  ...
  const score = calcScore(S.week, S.day);
  const prevScore = S.sessionScores[`${S.week - 1}_${S.day}`] || 0;
  const vol = day.mobilite ? 0 : Math.round(calcVolume(S.week, S.day));
  const m = Math.floor(S.sessionDuration / 60), sec = S.sessionDuration % 60;
  const durStr = `${m}:${sec.toString().padStart(2, '0')}`;
  const allExos = day.mobilite ? [] : [...(day.exos || []), ...(day.abdosExos || [])];
  let done = 0, total = 0;
